### Voice_Clone
Voice clone demo. In this repo, I used a model called GPT-SoVITS to clone my own voice. I will cover a little about literature and then we will go a step by step how I cloned my voice using GPT-SoVITS. I wanted to understand how it works. 


## Literature Review: Advances in Voice Cloning and Zero-Shot TTS

# Overview
Voice cloning, formally known as Multi-Speaker Text-to-Speech (TTS) aims to synthesize speech in the voice of a target speaker using  reference audio.
...
* In-Context Learning: This is its most famous feature. VALL-E can learn to clone a new voice from just a few second audio recording. This short recording, along with its text transcription, is called an "acoustic prompt."
...
• Recording: I prepared a custom dataset containing a few minutesof my English speech data.
• Slicing and Automatic Speech Recognition (ASR): The raw audio was sliced into short segments (2 to 10 seconds). This preprocessing step was critical because I used a local machine with out GPU. There are 2 model available in this step: Ali-Damo ASR (Chinese) and Faster Whisper (multi language). Since I use English, I chose Faster Whisper to do the slicing. The texts are also extracted at this stage (Labeling).
